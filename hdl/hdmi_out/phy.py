...
class _Clocking(Module, AutoCSR):
    def __init__(self, pads_vga, pads_dvi, external_clocking):
...
class Driver(Module, AutoCSR):
    def __init__(self, pack_factor, pads_vga, pads_dvi, external_clocking):
        fifo = _FIFO(pack_factor)
        self.submodules += fifo
        self.phy = fifo.phy
        self.busy = fifo.busy

        self.submodules.clocking = _Clocking(pads_vga, pads_dvi, external_clocking)
