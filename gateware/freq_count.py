...
class FreqCountCore(Module):
    def __init__(self, sample_width, full_width):
        ...
        self.comb += [
            self.clk_sync.inp.eq(self.ev_count.q),
...
class FrequencyCounter(Module, AutoCSR):
    def __init__(self, sample_clk_ticks, sample_width=6, full_width=32):
        ...
        self.submodules.core = core = FreqCountCore(sample_width, full_width)

        self.comb += [
            self.value.status.eq(core.count_latched),
            self.num_events.status.eq(core.count_curr),
            self.last_inc.status.eq(core.sampler.inc)
        ]
