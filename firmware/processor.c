...
static void fb_clkgen_write(int cmd, int data)
{
	...
	int word;
	word = (data << 2) | cmd;
	hdmi_out_driver_clocking_cmd_data_write(word);
	hdmi_out_driver_clocking_send_cmd_data_write(1);
	while(hdmi_out_driver_clocking_status_read() & CLKGEN_STATUS_BUSY);
#endif
}
...
{
	processor_hdmi_out_source = VIDEO_IN_HDMI_IN0;
...
#ifdef CSR_HDMI_OUT_BASE
	hdmi_out_core_initiator_enable_write(0);
	hdmi_out_driver_clocking_pll_reset_write(1);
#endif
#ifdef CSR_HDMI_OUT1_BASE
	hdmi_out1_core_initiator_enable_write(0);
#endif
...
#ifdef CSR_HDMI_OUT_BASE
	hdmi_out_driver_clocking_pll_reset_write(0);
...
void processor_update(void)
{
	...
	/*  hdmi_out */
#ifdef CSR_HDMI_IN0_BASE
	if(processor_hdmi_out_source == VIDEO_IN_HDMI_IN0)
		hdmi_out_core_initiator_base_write(hdmi_in0_framebuffer_base(hdmi_in0_fb_index));
#endif
#ifdef CSR_HDMI_IN1_BASE
