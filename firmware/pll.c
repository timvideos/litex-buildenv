...
{
#if defined(CSR_HDMI_OUT_BASE) || defined(CSR_HDMI_IN0_BASE) || defined(CSR_HDMI_IN1_BASE)
	int i;
#endif
	/*
	...
	for(i=6;i<32-5;i++) {
		hdmi_out_driver_clocking_pll_adr_write(i);
		hdmi_out_driver_clocking_pll_dat_w_write(data[i]);
		hdmi_out_driver_clocking_pll_write_write(1);
		while(!hdmi_out_driver_clocking_pll_drdy_read());
	}
#endif
...
void pll_dump(void)
{
	...
	for(i=0;i<32;i++) {
		hdmi_out_driver_clocking_pll_adr_write(i);
		hdmi_out_driver_clocking_pll_read_write(1);
		while(!hdmi_out_driver_clocking_pll_drdy_read());
		printf("%04x ", hdmi_out_driver_clocking_pll_dat_r_read());
	}
	printf("\r\n");
#endif
#ifdef CSR_HDMI_IN0_BASE
